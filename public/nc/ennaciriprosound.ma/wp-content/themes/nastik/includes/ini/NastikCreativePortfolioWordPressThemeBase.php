<?php
if(!class_exists("NastikCreativePortfolioWordPressThemeBase")) {
	class NastikCreativePortfolioWordPressThemeBase {
    	public $key = "F2960DF55F883D67";
    	private $product_id = "3";
    	private $product_base = "nastik";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_nastik_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=nastik_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
            ...
            $responseObj->license_key   = '86b4ea8e-7317-4ecb-8670-f974f0fb7102';
			return true;
