        ...
        <div class="row">
            <div class="col-lg-3 col-md-4">
                <aside class="sidebar-widget ">
                    ...
                    <div class="single-widget recent-posts-widget">
                        <h3 class="widget-title">{{ __('main.Recent Posts') }}</h3>
                        <ul>
                    ...
                    <div class="single-widget tag-widget">
                        <h3 class="widget-title">{{ __('main.Popular Tag') }}</h3>
                        <ul>
            ...
            <div class="col-lg-9 col-md-8">
                <div class="blog-details-area">
                    ...
                    <div class="post-tag">
                        <ul>
                            <li><span>{{ __('main.Popular Tag') }}:</span></li>
                            @foreach (explode(',', $news->tags) as $tag)
                    ...
                    <ul class="pagination-wrap">
                        <li class="previous">
                            ...
                            <a href="{{ route('front.single.news', $previous_news->slug) }}">
                                <span> <i class="flaticon-left-arrow"></i> {{ __('main.Previous Post') }}</span>
                                <h4>{{ Str::limit($previous_news->title, '40', '...') }} </h4>
                            </a>
                            @endif
                        </li>
                        <li class="next">
                            ...
                            <a href="{{ route('front.single.news', $next_news->slug) }}">
                                <span>{{ __('main.Next Post') }} <i class="flaticon-right-arrow"></i></span>
                                <h4>{{ Str::limit($next_news->title, '40', '...') }} </h4>
                            </a>
                            @endif
                        </li>
                    </ul>
                ...
                <div class="comment-area">
                    <h2 class="comment-title">{{ __('main.Comment') }} ({{ App\Models\Admin\Comment::where('status', 1)->count() }})</h2>
                ...
                <div class="comment-form" id="comment-form">
                    <h2 class="form-title">{{ __('main.Leave a Replay') }}</h2>
                    @if (Auth::check())
                    <form action="{{ route('comment.store') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            <label for="message">{{ __('main.Your Messages') }}</label>
                            <textarea class="form-control comment-box" id="message" name="massage" rows="3" placeholder="{{ __('main.Your Messages') }}"></textarea>
                            <span class="text-danger">{{ __($errors->first('massage')) }}</span>
                            <input type="hidden" name="p_id" value="{{ $newsId->id }}">
                        </div>
                        <button type="submit" class="primary-btn">{{ __('main.Post Comment') }}</button>
                    </form>
                    @else
                    <h4>{{ __('main.Please') }} <a href="{{ route('signin') }}">{{ __('main.login') }}</a> {{ __('main.to leave your comment') }}</h4>
                    @endif
                </div>
            </div>
        </div>
