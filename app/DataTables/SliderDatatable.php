        ...
        return datatables()
            ...
            ->addColumn('image', function ($query) {
                if (isset($query->image)) {
                    return '<img src="' . asset(path_slider_image() . $query->image) . '"  class="wdh-90" alt="' . __('No Image') . '"/>';
                } else {
            ...
            ->addColumn(
                ...
                function ($data) {
                    ...
                    if (auth()->user()->can('slider-edit')) {
                        $btn = $btn . '<a href="' . route("slider.edit", $data->id) . '"><i class="ik ik-edit"></i></a>';
                    }
                    if (auth()->user()->can('slider-delete')) {
                        $btn = $btn . '<a class="delete" href="' . route("slider.delete", $data->id) . '"><i class="ik ik-trash-2"></i></a>';
                    }
                    return $btn;
                }
            )
            ->rawColumns(['image', 'action', 'big_heading', 'description', 'status']);
    }
